    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                ...
                let estadoProducto = objProducto.PRO_STATUS == 1 ? 
                '<span class="badge badge-success">Activo</span>' : 
                '<span class="badge badge-danger">Inactivo</span>';
                document.querySelector("#celCodigo").innerHTML      = objProducto.PRO_ID;
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                ...
                document.querySelector("#idProducto").value     = objProducto.PRO_ID;
                ...
                document.querySelector("#listStatus").value     = objProducto.PRO_STATUS;
                tinymce.activeEditor.setContent(objProducto.PRO_DESCRIPCION); 
                ...
                if(objProducto.images.length > 0){
                    ...
                    for (let p = 0; p < objProductos.length; p++) {
                        ...
                        htmlImage +=`<div id="div${key}">
                            ...
                            <button type="button" class="btnDeleteImage" onclick="fntDelItem('#div${key}')" imgname="${objProductos[p].IMA_IMAGEN}">
